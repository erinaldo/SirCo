Public Class frmPpalFacturasDiferidos
    'mreyes 12/Noviembre/2014   05:36 p.m.
    ...
    Private Sub Btn_Liquidacion_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_Liquidacion.Click
        Try
            ...
            If contChecks > 0 Then
                Dim myForm1 As New frmLiquidacionDiferidos
    ...
    Private Sub Btn_Factoraje_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_Factoraje.Click
        Try
            ...
            Dim objDataRow1 As Data.DataRow
            ''Dim Nopago As Integer
            ''If MsgBox("Desea cambiar a PUBLICADO  el Estatus de los Pagos de  Facturas seleccionadas?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Confirmación") = MsgBoxResult.No Then Exit Sub
            ''For i As Integer = 0 To DGrid.Rows.Count - 1
            ''    If DGrid.Rows(i).Cells("SelecR").Value = True Then
            ''        Using objCatCuentas As New BCL.BCLCatalogoCuentas(GLB_ConStringCipSis)
            ''            Nopago = Mid(DGrid.Rows(i).Cells("marca").Value.ToString.Trim(), 1, InStr(DGrid.Rows(i).Cells("marca").Value.ToString.Trim(), "/") - 1)
            ''            If Not objCatCuentas.usp_ActualizaPagoFacturaDiferidos(1, DGrid.Rows(i).Cells("idfoliosuc").Value.ToString.Trim(), "9", "2015-01-22", 0, Nopago) Then
            ''                'Throw New Exception("No se pudo actualizar el Estatus.")
            ''            Else
            ''                'MsgBox("Estatus Actualizado correctamente.", MsgBoxStyle.Information, "Confirmación")
            ''            End If
