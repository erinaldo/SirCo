        ...
        Try
            If blnAplicar = True Then
                ...
                Dim intCont As Integer = 0
                    ...
                    For i As Integer = 0 To objDataSetLiquidacion.Tables(0).Rows.Count - 1
                        ...
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            objCuentas.usp_ActualizaLiquidacionDet(1, idLiquidacion, idFolio, "", status, "")
                        End Using
                    ...
                    If intCont > 0 Then
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            objCuentas.usp_ActualizaEstatusLiquidacion(3, idLiquidacion, "AP", idCuenta, sumSubTotal, sumGastos, sumImpuesto, sumCargo, sumCredito, sumDescuento, sumTotal, Now.ToString("yyyy-MM-dd HH:mm:ss"), GLB_Idempleado)
                        End Using
                    ...
                    '''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                    objDataSet = objCuentas.usp_TraerLiquidacion(idLiquidacion, "1900-01-01", "1900-01-01", "INSERT INTO clasificacion (status) VALUES ('AP')", "INSERT INTO clasificacionB (status) VALUES ('AP')", "INSERT INTO clasificacionB (status) VALUES ('AP')", "INSERT INTO clasificacionB (status) VALUES ('AP')", "")
                End Using
                    Dim Tipo As String = ""
                    If objDataSet.Tables(0).Rows.Count > 0 Then
                    ...
                    End Using
                        ...
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            objCuentas.usp_ActualizaLiquidacionDet(2, idLiquidacion, idFolio, noCheque, 0, "")
                        End Using
            ...
            If blnAplicar = False Then
                ...
                If contador > 0 Then
                    Dim encabezado As Boolean = False
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        encabezado = objCuentas.usp_CapturaLiquidacion(0, sumSubTotal, sumGastos, sumImpuesto, sumCargo, sumCredito, SumDescuento, sumTotal, "GE", TipoRF, Now.ToString("yyyy-MM-dd HH:mm:ss"), GLB_Idempleado, fechaVencimientoIni, fechaVencimientoFin, sumImprocedente)
                    End Using
                    If encabezado = False Then
                        MessageBox.Show("Error al guardar, por favor intenta de nuevo", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        Exit Sub
                    ...
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        idLiquidacion = CInt(objCuentas.usp_TraerIdLiquidacion().Tables(0).Rows(0).Item("idliquidacion").ToString)
                    End Using
                    'Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                    '    objDataSetFacturasAux = objCuentas.usp_TraerLiquidacionDetalle(4, idLiquidacion, "")
                    'End Using
                    For i As Integer = 0 To objDataSetAux.Tables(0).Rows.Count - 1
                        ...
                        End Using
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            objCuentas.usp_CapturaLiquidacionDet(idLiquidacion, idFolio, "", Proveedor, Factura, 1, FechaFactura, Fecha, subtotal, gastos, impuesto, cargo, credito, descuento, Total, Improcedente)
                        End Using
            ...
            Try
                If blnAplicar = True Then
                    ...
                    If EmpezoActualizar4 = True Then
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            objCuentas.usp_ActualizaEstatusLiquidacion(1, objDataSetLiquidacion.Tables(0).Rows(0).Item("idliquidacion").ToString, "RV", 0, 0, 0, 0, 0, 0, 0, 0, "", 0)
                        End Using
                    End If
                    ...
                    If EmpezoActualizar3 = True Then
                        ...
                        For i As Integer = 0 To objDataSetLiquidacionAux.Tables(0).Rows.Count - 1
                            Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                                objCuentas.usp_ActualizaLiquidacionDet(2, objDataSetLiquidacionAux.Tables(0).Rows(i).Item("idliquidacion").ToString, objDataSetLiquidacionAux.Tables(0).Rows(i).Item("idfolio").ToString, objDataSetLiquidacionAux.Tables(0).Rows(i).Item("nocheque").ToString, 0, "")
                            End Using
                        Next
                    End If
