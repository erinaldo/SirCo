Public Class frmLiquidacionFactoraje
    Dim Sql As String
        ...
        Try
            If blnAplicar = True Then
                ...
                If PagarRemision = True Then
                    idCuenta = 0
                Else
                    If MessageBox.Show("Esta seguro que desea aplicar la liquidación?", "CONFIRMACIÓN", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.No Then
                        Exit Sub
                    End If
                ...
                Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                    objDataSetFacturasAux = objCuentas.usp_TraerLiquidacionDetalleFactoraje(4, idLiquidacion, "")
                End Using
                Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                ...
                For i As Integer = 0 To objDataSetLiquidacion.Tables(0).Rows.Count - 1
                    ...
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        objCuentas.usp_ActualizaLiquidacionDetFactoraje(1, idLiquidacion, idFolio, "", status, "")
                    End Using
                ...
                If intCont > 0 Then
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        objCuentas.usp_ActualizaEstatusLiquidacionFACTORAJE(3, idLiquidacion, "AP", idCuenta, sumSubTotal, sumGastos, sumImpuesto, sumCargo, sumCredito, sumDescuento, sumTotal, Now.ToString("yyyy-MM-dd HH:mm:ss"), GLB_Idempleado)
                    End Using
                ...
                '''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                    objDataSet = objCuentas.usp_TraerLiquidacionFactoraje(idLiquidacion, "1900-01-01", "1900-01-01", "INSERT INTO clasificacion (status) VALUES ('AP')", "")
                End Using
                Dim Tipo As String = ""
                If objDataSet.Tables(0).Rows.Count > 0 Then
                ...
                For i As Integer = 0 To objDataSetLiquidacion.Tables(0).Rows.Count - 1
                    ...
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        objCuentas.usp_ActualizaLiquidacionDetFactoraje(2, idLiquidacion, idFolio, noCheque, 0, "")
                    End Using
            ...
            If blnAplicar = False Then
                If MessageBox.Show("Esta seguro que desea generar la liquidación para Factoraje?", "CONFIRMACIÓN", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.No Then
                    Exit Sub
                End If
                Dim sumSubTotal As Double = 0
                Dim sumGastos As Double = 0
                ...
                Dim sw_aceptafactoraje As Boolean = False
                EmpezoActualizar1 = False
                EmpezoActualizar2 = False
                EmpezoActualizar3 = False
                If blnCheck = True Then
                    ...
                    For i As Integer = 0 To objDataSetLiquidacion.Tables(0).Rows.Count - 1
                        Using objProveedor As New BCL.BCLCatalogoProveedores(GLB_ConStringCipSis)
                            objDataSet = objProveedor.usp_TraerProveedorNoPagos(Mid(objDataSetLiquidacion.Tables(0).Rows(i).Item("proveedor").ToString.Trim, 1, 3)) 'checar variable
                            If objDataSet.Tables(0).Rows.Count > 0 Then
                                'Generar el emparrillado de pagos.
                                If objDataSet.Tables(0).Rows(0).Item("aceptafactoraje").ToString > 0 Then
                                    sw_aceptafactoraje = True
                                Else
                                    sw_aceptafactoraje = False

                                End If
                            End If
                        End Using
                        If sw_aceptafactoraje = True Then
                            'If objDataSet.Tables(0).Rows(0).Item("pagado").ToString.Trim = 1 Then
                            'ir a poner el factoraje el folio
                            Using objCatCuentas As New BCL.BCLCatalogoCuentas(GLB_ConStringCipSis)
                                If Not objCatCuentas.usp_GeneraFactprovFactoraje(Mid(objDataSetLiquidacion.Tables(0).Rows(i).Item("proveedor").ToString.Trim, 1, 3), pub_TraerIdFolioBulto(objDataSetLiquidacion.Tables(0).Rows(i).Item("idfoliosuc").ToString.Trim)) Then
                            ...
                            'End If
                        Else
                            MsgBox("El proveedor seleccionado no acepta factoraje, verifique con compras.", MsgBoxStyle.Critical, "ERROR")
                            Exit Sub
                        End If
                    Next
                Else
                            ...
                            If FecVen >= fechaVencimientoIni And FecVen <= fechaVencimientoFin Then
                                ...
                                If Pagado = TipoPago And Tipo = TipoRF Then
                                    If Txt_Proveedor.Text.Trim = "" Then
                                        ...
                                        Dim descuento As Double = objDataSetLiquidacion.Tables(0).Rows(i).Item("descuento").ToString
                                        Dim Total As Double = objDataSetLiquidacion.Tables(0).Rows(i).Item("importe").ToString
                                        Dim ProveedorB As Integer = Mid(objDataSetLiquidacion.Tables(0).Rows(i).Item("proveedor").ToString, 1, 3)
                ...
                If contador > 0 Then
                    Dim encabezado As Boolean = False
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        encabezado = objCuentas.usp_CapturaLiquidacionFactoraje(0, sumSubTotal, sumGastos, sumImpuesto, sumCargo, sumCredito, SumDescuento, sumTotal, "GE", TipoRF, Now.ToString("yyyy-MM-dd HH:mm:ss"), GLB_Idempleado, fechaVencimientoIni, fechaVencimientoFin, SumImprocedente)
                    End Using
                    If encabezado = False Then
                        MessageBox.Show("Error al guardar, por favor intenta de nuevo", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        Exit Sub
                    ...
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        idLiquidacion = CInt(objCuentas.usp_TraerIdLiquidacionFactoraje().Tables(0).Rows(0).Item("idliquidacion").ToString)
                    End Using
                    'Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                    ...
                    For i As Integer = 0 To objDataSetAux.Tables(0).Rows.Count - 1
                        ...
                        End Using
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            objCuentas.usp_CapturaLiquidacionDetFactoraje(idLiquidacion, idFolio, "", Proveedor, Factura, 1, FechaFactura, Fecha, subtotal, gastos, impuesto, cargo, credito, descuento, Total, Improcedente)
                        End Using
            ...
            Try
                If blnAplicar = True Then
                    ...
                    If EmpezoActualizar4 = True Then
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            objCuentas.usp_ActualizaEstatusLiquidacionFactoraje(1, objDataSetLiquidacion.Tables(0).Rows(0).Item("idliquidacion").ToString, "RV", 0, 0, 0, 0, 0, 0, 0, 0, "", 0)
                        End Using
                    End If
                    ...
                    If EmpezoActualizar3 = True Then
                        ...
                        For i As Integer = 0 To objDataSetLiquidacionAux.Tables(0).Rows.Count - 1
                            Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                                objCuentas.usp_ActualizaLiquidacionDetFactoraje(2, objDataSetLiquidacionAux.Tables(0).Rows(i).Item("idliquidacion").ToString, objDataSetLiquidacionAux.Tables(0).Rows(i).Item("idfolio").ToString, objDataSetLiquidacionAux.Tables(0).Rows(i).Item("nocheque").ToString, 0, "")
                            End Using
                        Next
                    End If
