Public Class frmLiquidacionDiferidos
    'mreyes 12/Noviembre/2014   06:30 p.m.
    Dim Sql As String
        ...
        Try
            If blnAplicar = True Then
                ...
                If PagarRemision = True Then
                    idCuenta = 0
                Else
                    If MessageBox.Show("Esta seguro que desea aplicar la liquidación de PAGOS DIFERIDOS?", "CONFIRMACIÓN", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.No Then
                        Exit Sub
                    End If
                ...
                Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                    objDataSetFacturasAux = objCuentas.usp_TraerLiquidacionDetalleDiferidos(4, idLiquidacion, "")
                End Using
                Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                ...
                For i As Integer = 0 To objDataSetLiquidacion.Tables(0).Rows.Count - 1
                    ...
                    Dim NoPago As Integer = 0
                    'NoPago = Mid(objDataSetLiquidacion.Tables(0).Rows(i).Item("PAGO").ToString, 1, 1)
                    NoPago = Mid(objDataSetLiquidacion.Tables(0).Rows(i).Item("PAGO").ToString, 1, InStr(objDataSetLiquidacion.Tables(0).Rows(i).Item("pago").ToString, "/") - 1)
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        objCuentas.usp_ActualizaLiquidacionDetDiferidos(1, idLiquidacion, idFolio, "", status, "", NoPago)
                    End Using
                    EmpezoActualizar1 = True
                    intCont += 1
                    If status = 0 Then
                        Using objCuentas As New BCL.BCLCatalogoCuentas(GLB_ConStringCipSis)
                            objCuentas.usp_ActualizaPagoFacturaDiferidos(1, idFolio, 3, "", 0, NoPago)
                        End Using
                    Else
                ...
                If intCont > 0 Then
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        objCuentas.usp_ActualizaEstatusLiquidacionDiferidos(3, idLiquidacion, "AP", idCuenta, sumSubTotal, sumGastos, sumImpuesto, sumCargo, sumCredito, sumDescuento, sumTotal, Now.ToString("yyyy-MM-dd HH:mm:ss"), GLB_Idempleado)
                    End Using
                ...
                '''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                    objDataSet = objCuentas.usp_TraerLiquidacionDiferidos(idLiquidacion, "1900-01-01", "1900-01-01", "INSERT INTO clasificacion (status) VALUES ('AP')", "INSERT INTO clasificacionB (status) VALUES ('AP')", "INSERT INTO clasificacionBb (status) VALUES ('AP')", "")
                End Using
                Dim Tipo As String = ""
                If objDataSet.Tables(0).Rows.Count > 0 Then
                ...
                For i As Integer = 0 To objDataSetLiquidacion.Tables(0).Rows.Count - 1
                    ...
                    NoPago = Mid(objDataSetLiquidacion.Tables(0).Rows(i).Item("pago").ToString.Trim, 1, 1)
                    ...
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        objCuentas.usp_ActualizaLiquidacionDetDiferidos(2, idLiquidacion, idFolio, noCheque, 0, "", NoPago)
                    End Using
                    EmpezoActualizar3 = True
                    Using objCuentas As New BCL.BCLCatalogoCuentas(GLB_ConStringCipSis)
                        objCuentas.usp_ActualizaPagoFacturaDiferidos(2, idFolio, 1, GLB_FechaHoy.ToString("yyyy-MM-dd"), idLiquidacion, NoPago)
                    End Using
            ...
            If blnAplicar = False Then
                If MessageBox.Show("Esta seguro que desea generar la liquidación de PAGOS DIFERIDOS?", "CONFIRMACIÓN", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.No Then
                    Exit Sub
                End If
                Dim sumSubTotal As Double = 0
                Dim sumGastos As Double = 0
                ...
                If contador > 0 Then
                    Dim encabezado As Boolean = False
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        encabezado = objCuentas.usp_CapturaLiquidacionDiferidos(0, sumSubTotal, sumGastos, sumImpuesto, sumCargo, sumCredito, SumDescuento, sumTotal, "GE", TipoRF, Now.ToString("yyyy-MM-dd HH:mm:ss"), GLB_Idempleado, fechaVencimientoIni, fechaVencimientoFin, SumImprocedente)
                    End Using
                    If encabezado = False Then
                        MessageBox.Show("Error al guardar, por favor intenta de nuevo", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        Exit Sub
                    ...
                    Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                        idLiquidacion = CInt(objCuentas.usp_TraerIdLiquidacionDiferidos().Tables(0).Rows(0).Item("idliquidacion").ToString)
                    End Using
                    'Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                    ...
                    For i As Integer = 0 To objDataSetAux.Tables(0).Rows.Count - 1
                        ...
                        'se arreglo pagos mreyes 10/Marzo/2015 10:11 a.m
                        Dim Nopago As Double = Mid(objDataSetLiquidacion.Tables(0).Rows(i).Item("marca").ToString, 1, InStr(objDataSetLiquidacion.Tables(0).Rows(i).Item("marca").ToString, "/") - 1)
                        Dim PagoS As Double = Mid(objDataSetLiquidacion.Tables(0).Rows(i).Item("marca").ToString, InStr(objDataSetLiquidacion.Tables(0).Rows(i).Item("marca").ToString, "/") + 1)
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            Factura = objCuentas.usp_TraerFacturaReferenc(idFolio).Tables(0).Rows(0).Item("referenc").ToString
                        End Using
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            objCuentas.usp_CapturaLiquidacionDetDiferidos(idLiquidacion, idFolio, "", Proveedor, Factura, 1, FechaFactura, Fecha, subtotal, gastos, impuesto, cargo, credito, descuento, Total, Nopago, PagoS, Improcedente)
                        End Using

                        Using objCuentas As New BCL.BCLCatalogoCuentas(GLB_ConStringCipSis)
                            objCuentas.usp_ActualizaPagoFacturaDiferidos(1, idFolio, 4, "", 0, Nopago)
                        End Using
                ...
                If blnAplicar = True Then
                    If EmpezoActualizar1 = True Then
                        For i As Integer = 0 To objDataSetFacturasAux.Tables(0).Rows.Count - 1
                            Using objCuentas As New BCL.BCLCatalogoCuentas(GLB_ConStringCipSis)
                                objCuentas.usp_ActualizaPagoFacturaDiferidos(1, objDataSetFacturasAux.Tables(0).Rows(i).Item("idfoliosuc").ToString, objDataSetFacturasAux.Tables(0).Rows(i).Item("pagado").ToString, "", 0, 0)
                            End Using
                        Next
                    ...
                    If EmpezoActualizar4 = True Then
                        Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                            objCuentas.usp_ActualizaEstatusLiquidacionDiferidos(1, objDataSetLiquidacion.Tables(0).Rows(0).Item("idliquidacion").ToString, "RV", 0, 0, 0, 0, 0, 0, 0, 0, "", 0)
                        End Using
                    End If
                    ...
                    If EmpezoActualizar3 = True Then
                        For i As Integer = 0 To objDataSetFacturasAux.Tables(0).Rows.Count - 1
                            Using objCuentas As New BCL.BCLCatalogoCuentas(GLB_ConStringCipSis)
                                objCuentas.usp_ActualizaPagoFacturaDiferidos(2, objDataSetFacturasAux.Tables(0).Rows(i).Item("idfoliosuc").ToString, objDataSetFacturasAux.Tables(0).Rows(i).Item("pagado").ToString, objDataSetFacturasAux.Tables(0).Rows(i).Item("fecpag").ToString, objDataSetFacturasAux.Tables(0).Rows(i).Item("idliquidacion").ToString, 0)
                            End Using
                        Next
                        For i As Integer = 0 To objDataSetLiquidacionAux.Tables(0).Rows.Count - 1
                            Using objCuentas As New BCL.BCLLiquidacion(GLB_ConStringCipSis)
                                objCuentas.usp_ActualizaLiquidacionDetDiferidos(2, objDataSetLiquidacionAux.Tables(0).Rows(i).Item("idliquidacion").ToString, objDataSetLiquidacionAux.Tables(0).Rows(i).Item("idfolio").ToString, objDataSetLiquidacionAux.Tables(0).Rows(i).Item("nocheque").ToString, 0, "", Mid(objDataSetLiquidacionAux.Tables(0).Rows(i).Item("nocheque").ToString, 1, 1))
                            End Using
                        Next
                    End If
