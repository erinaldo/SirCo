Public Class frmPpalCancelaFactoraje
    ...
    Private Sub RellenaGrid(Optional ByVal Opcion1 As Integer = 0)
        Using objFactura As New BCL.BCLFacturas(GLB_ConStringCipSis)
            ...
            Try
                ...
                objDataSet = objFactura.usp_PpalCancelaFactoraje(Opcion, FactNueva, Sucursalb, IIf(FactInib = "F-" Or FactInib = "", "", "F-" & FactInib), IIf(FactFinb = "F-" Or FactFinb = "", "", "F-" & FactFinb), Marcab, _
                                                                   Proveedorb, sTATUSB1, FechaInib, _
    ...
    Sub InicializaGrid()
        ...
        Try
            ...
            DGrid.Columns(7).HeaderText = "Vence Proveedor"
            ...
            DGrid.Columns(18).HeaderText = "Vence Banco"
            DGrid.Columns(18).DisplayIndex = 8
    ...
    Private Sub Btn_Layout_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_Layout.Click
        'mreyes 24/Octubre/2014 09:45 a.m
        If Sw_NoRegistros = False Then Exit Sub

        Try
            If MsgBox("Esta seguro de querer cancelar de Factoraje las facturas seleccionadas?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Confirmación") = MsgBoxResult.No Then Exit Sub
            Me.Cursor = Cursors.WaitCursor
            ...
            For Each row As DataGridViewRow In DGrid.Rows
                If row.Cells("SelecR").Value = True Then
                    Proveedor = Mid(row.Cells("proveedor").Value, 1, 3)
                    IdFolio = row.Cells("idfolio").Value
                    ...
                    Using objPedidos As New BCL.BCLPedidos(GLB_ConStringCipSis)
                        Try
                            ...
                            Using objCatCuentas As New BCL.BCLCatalogoCuentas(GLB_ConStringCipSis)
                                If Not objCatCuentas.usp_GeneraFactprovFactorajeCancela(Proveedor, IdFolio, GLB_Idempleado) Then
                                    'Throw New Exception("No se pudo actualizar el Estatus.")
                                Else
                                    'MsgBox("Estatus Actualizado correctamente.", MsgBoxStyle.Information, "Confirmación")
                                End If
            ...
            Using objCatCuentas As New BCL.BCLCatalogoCuentas(GLB_ConStringCipSis)
                ...
                End If
            End Using
            MsgBox("Se han Cancelado '" & Cont & "' Facturas ya en Factoraje.", MsgBoxStyle.Information, "Confirmación")
            Call RellenaGrid()
        Catch ExceptionErr As Exception
            MessageBox.Show(ExceptionErr.Message.ToString)
        End Try
